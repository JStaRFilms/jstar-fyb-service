...
interface SuggestionChipsProps {
    ...
    onProceed: () => void;
    isLoading: boolean;
    hasProvidedPhone?: boolean; // Manual fallback trigger
}
...
export function SuggestionChips({
    ...
    // Manual fallback: Phone provided but bot stuck (no confirmedTopic)
    if (hasProvidedPhone) {
        return (
            <div className="flex flex-wrap gap-2 mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <button
                    onClick={onProceed}
                    className="flex items-center gap-2 px-5 py-3 rounded-full bg-accent hover:bg-accent/90 text-white font-display font-bold uppercase tracking-wide shadow-[0_0_25px_rgba(236,72,153,0.4)] hover:shadow-[0_0_35px_rgba(236,72,153,0.6)] hover:scale-105 transition-all"
                >
                    <Rocket className="w-4 h-4" />
                    Skip to Builder (Got Your Info!)
                </button>
                <button
